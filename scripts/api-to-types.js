...
/**
 * api-to-types.js
 * Compiled JavaScript version of api-to-types.ts
 *
 * Generates TypeScript types from REST API responses and optionally validates
 * them against actual API endpoints.
 *
 * Usage:
 *   node api-to-types.js --url <endpoint>
 *   node api-to-types.js --input-json '<json>'
 *   cat response.json | node api-to-types.js
 */
...
function inferType(value, keyName, collector, opts) {
  if (value === null) return 'null';
  ...
  if (typeof value === 'object') {
    ...
    const definition = buildObjectDefinition(value, interfaceName, collector, opts);
...
function buildObjectDefinition(obj, name, collector, opts) {
...
function generateTypes(json, opts = {}) {
  ...
  if (Array.isArray(json)) {
    ...
    collector.set(rootName, `export type ${rootName} = ${inferPrimitiveType(json)};`);
  }
...
function formatOutput(types, header = true) {
  const parts = [];
  if (header) {
...
function isTypeCompatible(actual, declared) {
...
function validateAgainstInterface(json, iface) {
  const errors = [];
  const warnings = [];
...
function fetchUrl(url, opts) {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error(`Request timed out after ${opts.timeout || 15000}ms`));
    });
    req.on('error', reject);
    req.end();
  });
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--url':
      case '-u':        args.url = next; i++; break;
      case '--name':
      case '-n':        args.name = next; i++; break;
      case '--method':
      case '-m':        args.method = (next || 'GET').toUpperCase(); i++; break;
      case '--output':
      case '-o':        args.output = next; i++; break;
      case '--input-json': args.inputJson = next; i++; break;
      case '--headers':
      case '-H':
        try { args.headers = JSON.parse(next); } catch (e) { die(`--headers must be valid JSON: ${next}`); }
        i++;
        break;
    }
  }
...
function deriveTypeName(url) {
  try {
    ...
    return toPascalCase(seg.replace(/s$/, ''));
  } catch {
    return 'ApiResponse';
  }
}
...
async function main() {
  ...
  try {
    ...
    die(`Failed to parse JSON: ${String(rawJson).slice(0, 200)}`);
  }
  ...
  if (args.validate && args.url) {
    ...
    if (Array.isArray(parsed)) {
      ...
      validationJson = parsed[0];
    } else if (parsed !== null && typeof parsed === 'object') {
      validationJson = parsed;
    } else {
...
main().catch((err) => {
  console.error('[api-to-types] Unhandled error:', (err && err.message) ? err.message : err);
  process.exit(1);
});
